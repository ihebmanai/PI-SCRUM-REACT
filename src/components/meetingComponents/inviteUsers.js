...
import { Button, Card, CardBody,CardHeader,Table, CardFooter,FormGroup,Label,Badge, Col, Container, Form, Input, InputGroup, InputGroupAddon, InputGroupText, Row,Breadcrumb,BreadcrumbItem,CardGroup } from 'reactstrap';
...
class CreateMeetings extends Component {
    ...
    componentDidMount() {
      ...
      axios.get('http://localhost:3000/user').then((response)=>{
       ...
       console.log('count users'+this.state.users.length)
    ...
    axios.get('http://localhost:3000/meetings/id/'+this.props.match.params.id).then((response)=>{
       ...
       //  count : this.meeting.meeting.length
       })
    ...
    handleSubmit = (event) => {
      ...
      handleClickDelete(id,index) {
       ...
       this.setState([
         ...
         this.state.invited=this.state.invited+1
       ])
            console.log(index)
           });
               }
        ...
        return (
          <tr key={user._id.toString()}> 
            ...
            <td class="text-center">
              ...
              <Button color="primary"  className="btn-pill btn btn-primary " onClick={() => this.handleClickDelete(user._id,index)}>Invite</Button>
      ...
      return (
        ...
        <div className="app-body">
          ...
          <div className="card-body">
            <p>Details about the meeting you wish invite members .. </p>
            <div className="bd-example">
              <dl className="row">
                <dt className="col-sm-3">Title:</dt>
                <dd className="col-sm-9">{this.state.meeting.title}.</dd>

                <dt className="col-sm-3">Date:</dt>
                <dd className="col-sm-9">
                  <p>{this.state.meeting.date} At {this.state.meeting.startDate} </p>
                  
                </dd>

                <dt className="col-sm-3">Type:</dt>
                <dd className="col-sm-9">{this.state.meeting.type}.</dd>

                <dt className="col-sm-3 text-truncate">Scrum Master : </dt>
                <dd className="col-sm-9">Mr Dhiaeddine.</dd>

                <dt className="col-sm-3 text-truncate">Duree : </dt>
                <dd className="col-sm-9">{this.state.meeting.duree}.</dd>
        ...
        <CardGroup className="mb-4">
          <Widget04 icon="icon-people" color="primary" header="6" value="6">All Employees</Widget04>
          <Widget04 icon="icon-user-follow" color="success" header={this.state.invited}value={this.state.invited*3}>Invited</Widget04>
          <Widget04 icon="icon-pie-chart" color="warning" header={this.state.count}value={this.state.count}>Not invited:</Widget04>
          ...
          <Col>
            <Card>
              <CardHeader>
                <i class="cui-code icons font-2xl d-block mt-4"></i> Here you can find All Developer in The society... ({this.state.users.length} Agents Disponible)
              </CardHeader>
          ...
          <center><Link to="/meeting" className="navbar-brand"> Click Here,To See all Meetings... </Link></center>
